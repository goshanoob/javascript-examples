...
// getData() - собирает информацию из форм на странице.
		...
		document.getElementById("saveTeamsResults").addEventListener("click", () => {
			...
			this.f1.teams.forEach((team, i) => {
				text += `${i + 1}. ${team.name} ${team.id},${team.racers},${team.shassi},`+
						`${team.engine},${team.budget},${team.result + team.points}\n`;
			});
			new FS().saveFile(text, "Команды");
		});
	}
...
class F1 {
	...
	getUniqueRows(array) {
		const uniques = [],	itemsFound = {}, items = [];
		array.forEach(item => {
	...
	calculateTeamsResults(){
		this.teams.forEach(team => {
			team.result = 
				...
				+ this.getResult(1, team.shassi) 
				+ this.getResult(2, team.engine);
