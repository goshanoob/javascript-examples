...
class Statistics {
    ...
    getMean(data = this.data) {
        ...
        return this.toRound(sum / data.length, 3);
    }
    ...
    clearData(){
        ...
        let limitMax = mean + square;
        let limitMin = mean - square;
        
        return this.data.filter(element => element <= limitMax && element >= limitMin);
    }
    
    // Очищенное среднее.
    getClearMean = () =>
        this.getMean(this.clearData());
}
